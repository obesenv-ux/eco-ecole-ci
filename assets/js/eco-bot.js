...
function determinerIntention(texteBrut) {
    ...
    if (txt.includes("qu'est") || txt.includes("quest") || txt.includes("c'est quoi") || txt.includes("c quoi") || txt.includes("definition") || txt.includes("signifie")) {
        return "WHAT";
    }
    if (txt.includes("combien") || txt.includes("nombre")) {
        return "NUMBER";
    }
    if (txt.includes("salut") || txt.includes("bonjour") || txt.includes("qui es tu") || txt.includes("t'appelles")) {
        return "WHO";
    }
    return "ANY";
}
...
function trouverMeilleureReponse(messageUtilisateur) {
    ...
    baseDeConnaissances.forEach(item => {
        ...
        // Alignment d'intention (Bonus / Pénalité)
        if (intentionU !== "ANY") {
